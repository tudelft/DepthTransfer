...
def update_cfg_from_args(env_cfg, args):
    if env_cfg is not None:
        ...
        if args.num_envs is not None:
            ...
            if env_cfg.env.num_maps is not None:
                env_cfg.env.num_maps = args.num_maps
    return env_cfg
...
def get_args(additional_parameters=[]):
    custom_parameters = [
        ...
        {"name": "--num_maps", "type": int, "default": "16", "help": "Number of environments to create. Overrides config file if provided."},
    ]
    # parse arguments
