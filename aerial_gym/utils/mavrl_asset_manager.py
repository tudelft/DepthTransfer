...
from aerial_gym.utils.poisson_distribution import PoissonDiscSampler
...
class MAVRLAssetManager:
    def __init__(self, config, device):
        ...
        self.num_maps = self.cfg.env.num_maps
        ...
        self.env_free_space = self.asset_config.free_space
        ...
        self.asset_type_to_dict_map = {
            ...
            "forest": self.cfg.forest_asset_params,
            "objects": self.cfg.object_asset_params,
            "cyberzoo": self.cfg.cyberzoo_asset_params,
            "fake_trees": self.cfg.fake_trees_asset_params,
            "left_wall": self.cfg.left_wall,
            "right_wall": self.cfg.right_wall,
            "right_wall_flip": self.cfg.right_wall_flip,
            "back_wall": self.cfg.back_wall,
            "front_wall": self.cfg.front_wall,
            "bottom_wall": self.cfg.bottom_wall,
            "top_wall": self.cfg.top_wall,
            "bottom_wall_forest": self.cfg.bottom_wall_forest,
            "left_wall_forest": self.cfg.left_wall_forest,
            "right_wall_forest": self.cfg.right_wall_forest,
            "back_wall_forest": self.cfg.back_wall_forest,
            "front_wall_forest": self.cfg.front_wall_forest,
            }
    ...
    def load_asset_tensors(self):
        ...
        for asset_key, include_asset in self.asset_config.include_asset_type.items():
            ...
            self.assets.append(asset_class)
        ...
        for env_bound_key, include_asset in self.asset_config.include_env_bound_type.items():
            ...
            self.assets.append(env_bound_class)
        

    def prepare_assets_for_simulation(self):
        # 
        self.num_assets_to_keep = 0
        asset_list = []
        for asset_key, include_asset in self.asset_config.include_asset_type.items():
            ...
            for file_name in file_list:
                asset_dict = {
                    ...
                    "color": color,
                    "create_texture": self.cfg.env.create_texture
                }
            ...
            self.assets_index.append(asset_class.num_assets)
    ...
    def randomize_pose(self, keep_num_obstacles = None):
        ...
        self.env_lower_bound = (self.env_lower_bound_max + self.env_lower_bound_min)/2.0
        self.env_upper_bound = (self.env_upper_bound_max + self.env_upper_bound_min)/2.0
        if self.asset_pose_tensor is None:
            return
        ...
        self.asset_pose_tensor[:, :3] = self.env_lower_bound + self.env_bound_diff * pos_ratio_euler_asbolute[:,:3]
        ...
        # self.asset_pose_tensor[:, :3] -= 1000.0
        ...
        if keep_num_obstacles is not None:
            ...
            self.asset_pose_tensor[sampled_obstacle_indices, :3] -= 1000.0
        return
    
    def poission_sample_obstacles(self, if_easy_start=False):
        # Sampled environment bounds
        radius = self.cfg.env.poisson_radius_area * np.random.random((self.num_maps, 1)) + self.cfg.env.poisson_radius_origin
        assets_type_num = len(self.assets_index) // self.num_maps
        self.env_lower_bound = (self.env_lower_bound_max + self.env_lower_bound_min)/2.0
        self.env_upper_bound = (self.env_upper_bound_max + self.env_upper_bound_min)/2.0
        env_lower_bound_obs = self.env_lower_bound[:2] + self.env_free_space
        env_upper_bound_obs = self.env_upper_bound[:2] - self.env_free_space
        self.env_bound_diff = env_upper_bound_obs - env_lower_bound_obs
        asset_min_state_tensor = self.asset_min_state_tensor.expand(self.num_maps, -1, -1)
        asset_max_state_tensor = self.asset_max_state_tensor.expand(self.num_maps, -1, -1)
        pos_ratio_euler_asbolute = asset_min_state_tensor + torch.rand_like(asset_min_state_tensor)*(asset_max_state_tensor - asset_min_state_tensor)
        if self.asset_pose_tensor is None:
            return
        asset_poisson_pose_tensor = torch.zeros((self.num_maps, self.env_actor_count, 6), dtype=torch.float, device=self.device, requires_grad=False)
        asset_poisson_pose_tensor[:, :-(self.env_actor_count - sum(self.assets_index[:assets_type_num])), :3] -= 1000.0
        asset_poisson_pose_tensor[:, -(self.env_actor_count - sum(self.assets_index[:assets_type_num])):, :] = self.asset_pose_tensor[-(self.env_actor_count - sum(self.assets_index[:assets_type_num])):, :]
        asset_poisson_pose_tensor[:, :, 3:6] = pos_ratio_euler_asbolute[:, :, 3:6]
        for i in range(self.num_maps):
            # Poisson disc sampler
            if if_easy_start:
                sampler = PoissonDiscSampler(self.env_bound_diff.cpu()[0], self.env_bound_diff.cpu()[1], self.cfg.env.poisson_radius_easy_start)
            else:
                sampler = PoissonDiscSampler(self.env_bound_diff.cpu()[0], self.env_bound_diff.cpu()[1], radius[i])
            sample_pose_np = np.array(list(sampler.samples()))
            sample_pose = torch.tensor(sample_pose_np, device=self.device)
            sample_pose = sample_pose[torch.randperm(sample_pose.shape[0])]
            asset_nums = sample_pose.shape[0]
            # print("Sampled asset nums: ", asset_nums)
            # print("Total asset nums: ", self.env_actor_count)
            asset_nums = min(asset_nums, self.env_actor_count)
            new_assets_index = [(x * asset_nums // self.env_actor_count) for x in self.assets_index[i*assets_type_num:(i+1)*assets_type_num]]
            start_asset_index = 0
            end_asset_index = 0
            start_sample_index = 0
            end_sample_index = 0
            for j in range(len(new_assets_index)):
                end_asset_index = start_asset_index + new_assets_index[j]
                end_sample_index = start_sample_index + new_assets_index[j]
                asset_poisson_pose_tensor[i, start_asset_index:end_asset_index, :2] = sample_pose[start_sample_index:end_sample_index, :] + env_lower_bound_obs[:2]
                asset_poisson_pose_tensor[i, start_asset_index:end_asset_index, 2] = 0.0
                start_asset_index += self.assets_index[i*assets_type_num + j]
                start_sample_index += new_assets_index[j]
        return asset_poisson_pose_tensor
