...
class MAVRLTaskAgileCfg(BaseConfig):
    seed = 10
    class env:
        num_envs = 128
        num_maps = 1
        ...
        env_spacing = 50.0  # not used with heightfields/trimeshes
        ...
        poisson_radius_origin = 2.5
        poisson_radius_area = 2.5
        poisson_radius_easy_start = 20.0
        flight_lower_bound = [-1.0, -26.0, 0.1]
        flight_upper_bound = [50.0, 26.0, 3.0]
        act_max = [1.0, 1.0, 0.0, 3.0]
        act_min = [-1.0, -1.0, -0.0, -3.0]
        goal_num_per_episode = 1
        create_texture = False
        goal_arrive_threshold = 0.8
    ...
    class FeasibilityCfg(BaseConfig):
        stabilization_attitude_ref_omega = [15.5, 15.5, 20.5]
        stabilization_attitude_ref_zeta = [13.8, 13.8, 13.2]
        stabilization_attitude_ref_max = [25.2, 25.2, 23.1]
        stabilization_attitude_ref_max_omege_dot = [25.2, 25.2, 22.8]
    ...
    class robot_spawning_config:
        ...
        max_position_ratio = [0.2, 0.9, 0.9] # max position as a ratio of the bounds after offset
    ...
    class logging:
        ...
        sampling_step = [5, 5, 3]
        lower_bound = [0.0, -25, 0.0]
        upper_bound = [50.0, 25, 6.0]
        trial_nums = 10
    ...
    class asset_config:
        ...
        env_lower_bound_min = [0.0, -25, 0.0] # lower bound for the environment space
        env_lower_bound_max = [0.0, -25, 0.0] # lower bound for the environment space
        env_upper_bound_min = [50.0, 25, 4.0] # upper bound for the environment space
        env_upper_bound_max = [50.0, 25, 4.0] # upper bound for the environment space
        free_space = 3.0 # free space around the environment bounds
    ...
    class panel_asset_params(asset_state_params):
        ...
        min_euler_angles = [-np.pi/6.0, -np.pi/6.0, -np.pi/3.0] # min euler angles
        max_euler_angles = [np.pi/6.0, np.pi/6.0, np.pi/3.0] # max euler angles
    ...
    class top_wall(asset_state_params):
        ...
        min_position_ratio = [0.5, 0.5, 2.0] # min position as a ratio of the bounds
        max_position_ratio = [0.5, 0.5, 2.0] # max position as a ratio of the bounds
