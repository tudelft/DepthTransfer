...
class LeePositionController:
    def __init__(self, K_pos_tensor, K_vel_tensor, K_rot_tensor, K_angvel_tensor):
        ...
        self.K_pos_tensor_current = K_pos_tensor.clone()
        self.K_vel_tensor_current = K_vel_tensor.clone()
        self.K_rot_tensor_current = K_rot_tensor.clone()
        self.K_angvel_tensor_current = K_angvel_tensor.clone()
    
    def randomize_params(self, env_ids=None):
        if env_ids is None:
            env_ids = torch.arange(self.K_pos_tensor.shape[0])
        self.K_pos_tensor_current[env_ids] = (self.K_pos_tensor + ((torch.rand_like(self.K_pos_tensor) - 0.5) * 2) * 0.1)[env_ids]
        self.K_vel_tensor_current[env_ids] = (self.K_vel_tensor + ((torch.rand_like(self.K_vel_tensor) - 0.5) * 2) * 0.1)[env_ids]
        self.K_rot_tensor_current[env_ids] = (self.K_rot_tensor + ((torch.rand_like(self.K_rot_tensor) - 0.5) * 2) * 1.0)[env_ids]
    ...
    def __call__(self, robot_state, command_actions):
        ...
        accel_command = self.K_pos_tensor_current * pos_error - self.K_vel_tensor_current * robot_state[:, 7:10]
        accel_command[:, 2] += 1
