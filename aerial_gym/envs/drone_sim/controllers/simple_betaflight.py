...
class SimpleBetaflight:
    ...
    def compute(self, ang_vel: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # derivative term
        d_err_ang_vel = self.dterm_lpf.get_output()
...
def _compute_pid_sum_script(
    ...
    d_err_ang_vel: torch.Tensor,
    des_ang_vel: torch.Tensor,
) -> torch.Tensor:
    # PID sum and clamp
    pid_sum = (
        kp * err_ang_vel + ki * int_err_ang_vel - kd * d_err_ang_vel + kff * des_ang_vel
    )
