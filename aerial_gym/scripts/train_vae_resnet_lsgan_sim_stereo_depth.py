...
class AttentionAutoEncoderSimStereoDepth:
    def __init__(self, args) -> None:
        ...
        # load real data
        # self.dataset_real = RosbagDataset('../datasets', ['/camera/depth/image_rect_raw', '/camera/infra1/image_rect_raw'], 
        #                                   transform=self.transform_train, with_gray=False, train=True)
        # self.real_data_loader = torch.utils.data.DataLoader(self.dataset_real, batch_size=args.batch_size, shuffle=True, num_workers=8)

        # load stereo depth data
        self.dataset_stereo = _RolloutDataset('../saved/dataset_outdoor_stereo_new',
                                                self.transform_train, file_num=3, device=device, train=True)
        self.stereo_loader = torch.utils.data.DataLoader(
            self.dataset_stereo, batch_size=args.batch_size, shuffle=True, num_workers=8)
        ...
        self.avae_dir = join(args.logdir, 'vae_resnet_16ch_sim_stereo_depth')
    ...
    def train(self, epoch):
        ...
        self.dataset_stereo.load_next_buffer()
        train_d_loss = 0
        train_g_loss = 0
        len_syn = len(self.dataset_train)
        len_stereo = len(self.dataset_stereo)
        for batch_idx, (batch1, batch2) in enumerate(itertools.zip_longest(self.train_loader, self.stereo_loader, fillvalue=None)):
            ...
            if self.args.use_max_pooling:
                batch1 = min_pool2d(batch1, kernel_size=7, stride=1, padding=3)
                batch2 = min_pool2d(batch2, kernel_size=3, stride=1, padding=1)

            save_image(batch2, join(self.avae_dir, 'samples', 'train_' + str(epoch) + '.png'))

            if self.it_g % 20 == 0:
                self.optimizer_disc.zero_grad()
                d_loss = self.training_step(batch2, batch1, 1)
                d_loss.backward()
            ...
            if self.it_d % 1 == 0:
                self.optimizer.zero_grad()
                g_loss = self.training_step(batch2, batch1, 0)
                g_loss.backward()
                self.optimizer.step()
        ...
        average_d_loss = (train_d_loss / len_syn) if len_syn > len_stereo else (train_d_loss / len_stereo)
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.test_loader):
                ...
                save_image(data, join(self.avae_dir, 'samples', 'gt_depth_' + str(epoch) + '.png'))
                if self.args.use_max_pooling:
                    input = min_pool2d(input, kernel_size=7, stride=1, padding=3)
...
def main():
    ...
    ae = AttentionAutoEncoderSimStereoDepth(args)
    ae.learn()
