...
def main():
    ...
    collect_env = mavrl_vec_env_v2.MavrlEnvVecV2(env)

    rsg_root = os.path.dirname(os.path.abspath(__file__))
    log_dir = rsg_root + "/../saved/"
    device = collect_env.device
    ...
    if not args.nocontrol:
        policy_weight = log_dir + "resnet_vae_with_lstm_normalized/Policy/iter_{0:05d}.pth".format(args.iter)
        saved_variables = torch.load(policy_weight, map_location=device)

        lstm_weights = log_dir + "LSTM_1/Policy/iter_03950.pth"
        lstm_variables = torch.load(lstm_weights, map_location=device)

        if env_cfg.LatentSpaceCfg.normalize_obs:
            env_rms = log_dir + "resnet_vae_with_lstm_normalized/RMS/iter_{0:05d}.pth".format(args.iter)
            collect_env.load_rms(env_rms, env_cfg.env.num_envs)

        encoder = log_dir + "resnet_vae_with_lstm_normalized/Encoder/iter_{0:05d}.pth".format(args.iter)
        encoder_variables = torch.load(encoder, map_location=device)
        pre_control_policy = {
            ...
            'lstm_actor.weight_ih_l0': lstm_variables["state_dict"]['lstm_actor.weight_ih_l0'],
            'lstm_actor.weight_hh_l0': lstm_variables["state_dict"]['lstm_actor.weight_hh_l0'],
            'lstm_actor.bias_ih_l0': lstm_variables["state_dict"]['lstm_actor.bias_ih_l0'],
            'lstm_actor.bias_hh_l0': lstm_variables["state_dict"]['lstm_actor.bias_hh_l0'],
        }
