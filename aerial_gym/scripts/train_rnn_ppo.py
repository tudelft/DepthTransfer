...
def main():
    add_args = [{"name": "--train", "type": bool, "default": True, "help": "Train the policy or evaluate the policy"},
        ...
        {"name": "--load_encoder", "type": bool, "default": False, "help": "if load encoder"},
        {"name": "--rollouts", "type": int, "default": 1000, "help": "Number of rollouts"},
        {"name": "--logdir", "type": str, "default": "../saved/LSTM_Encoder_0/", "help": "Directory where results are logged"},
        {"name": "--visual_recon", "type": bool, "default": False, "help": "if use visual reconstruction"},
    ]
    ...
    if args.load_encoder:
        curriculum_steps = 2e6
    else:
        env_cfg.robot_asset.collision_mask = 1
        curriculum_steps = 0
    env, env_cfg = task_registry.make_env(name=args.task, args=args, env_cfg=env_cfg)
    if args.visual_recon:
        train_env = mavrl_vec_rnn_env.MavrlEnvVecRNN(env, reconstruction_members=[True, True, False])
    else:
        train_env = mavrl_vec_rnn_env.MavrlEnvVecRNN(env)

    rsg_root = os.path.dirname(os.path.abspath(__file__))
    log_dir = rsg_root + "/../saved"
    device = train_env.device
    if args.load_encoder:
        vae_logdir = args.logdir
        rnn_vae_file = join(vae_logdir, 'Policy', 'iter_{:05d}.pth'.format(args.iter))
        assert exists(rnn_vae_file), "No trained Encoder in the logdir..."
        ...
        for key in state_vae['state_dict'].keys():
            if key.startswith('features_extractor.'):
                ...
                encoder[new_key] = state_vae['state_dict'][key]
            elif key.startswith('feature_decoder0.'):
                new_key = key.replace('feature_decoder0.', '')
                decoder[new_key] = state_vae['state_dict'][key]
            elif key.startswith('mu_linear.'):
                new_key = key.replace('mu_linear.', '')
                mu_linear[new_key] = state_vae['state_dict'][key]
            elif key.startswith('lstm_actor.'):
                new_key = key.replace('lstm_actor.', '')
                lstm_weights[new_key] = state_vae['state_dict'][key]
        if args.visual_recon:
            train_env.load_features_extractor(encoder, lstm_weights, mu_linear, decoder)
        else:
            train_env.load_features_extractor(encoder, lstm_weights, mu_linear)
