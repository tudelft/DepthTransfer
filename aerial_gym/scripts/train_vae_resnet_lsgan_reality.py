...
class AttentionAutoEncoderRealityDepth:
    def __init__(self, args) -> None:
        ...
        self.avae_dir = join(args.logdir, 'vae_resnet_16ch_reality')
        if not exists(self.avae_dir):
            mkdir(self.avae_dir)
            mkdir(join(self.avae_dir, 'samples'))

        self.reload_file = join(join(args.logdir, 'vae_resnet_16ch_reality'), 'best.tar')
    ...
    def learn(self):
        for epoch in range(1, self.args.epochs + 1):
            ...
            self.dataset_real.load_next_buffer()

            # Get the lengths of both DataLoaders
            len_syn = len(self.train_loader)
            len_real = len(self.real_data_loader)
            ...
            syn_iter = iter(self.train_loader)
            real_iter = iter(self.real_data_loader)
            ...
            # End of while loop, go to the next epoch
            average_d_loss = (train_d_loss / len_syn) if len_syn > len_real else (train_d_loss / len_real)
            average_g_loss = (train_g_loss / len_syn) if len_syn > len_real else (train_g_loss / len_real)
...
def main():
    ...
    ae = AttentionAutoEncoderRealityDepth(args)
    ae.learn()
