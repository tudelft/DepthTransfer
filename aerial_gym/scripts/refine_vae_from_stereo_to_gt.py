...
class DomainAdaptationFromLatent:
    def __init__(self, args) -> None:
        ...
        if self.args.zoo_task:
            # load target dataset
            self.dataset_target = _RolloutDataset('../saved/zoo_dataset_stereo_new',
            ...
            # load target dataset
            self.dataset_target = _RolloutDataset('../saved/dataset_outdoor_stereo_new',
                                                    self.transform_train, device=device, train=True)
            # load source data
        ...
        self.optimizer_disc = optim.Adam(self.discriminator.parameters(), lr=3e-4, betas=(0.5, 0.9))
        ...
        self.avae_dir = join(args.logdir, 'vae_resnet_from_latent_zoo')
    ...
    def train_vae(self, epoch):
        ...
        for batch_idx, (batch1, batch2) in enumerate(itertools.zip_longest(self.target_loader, self.gt_loader, fillvalue=None)):
            ...
            total_loss = target_loss + 10.0 * domain_loss
...
def main():
    ...
    da = DomainAdaptationFromLatent(args)
    da.learn()
